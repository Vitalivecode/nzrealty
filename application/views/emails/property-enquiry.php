                  ...
                  <table style="font-family: 'Montserrat',Arial,sans-serif; width: 100%;" width="100%" cellpadding="0" cellspacing="0" role="presentation">
                    <tr>
                      <td class="sm-px-24" style="--bg-opacity: 1; background-color: #ffffff; border-radius: 4px; font-family: Montserrat, -apple-system, 'Segoe UI', sans-serif; font-size: 14px; line-height: 24px; padding: 48px; text-align: left; --text-opacity: 1; color: #626262;" align="left">
                        ...
                        <p style="font-weight: 700; font-size: 20px; margin-top: 0; --text-opacity: 1; color: #ff5850;"><?=ucfirst($agentDetails[0]->name);?>!</p>
                        <p style="margin: 0 0 24px;">
                          <?=$subject;?> <strong><?=($propertyDetails != false)?$propertyDetails[0]->address:'';?></strong> on <strong><?=date('d-m-Y h:i A', strtotime($datetime));?></strong><br/>
                          <strong>Username:</strong> <?=ucfirst($userDetails['name']);?><br/>
                          <strong>Email:</strong> <?=$userDetails['email'];?><br/>
                          <strong>Contact:</strong> <?=$userDetails['phone'];?><br/>
                          <?php if(isset($userDetails['day_date']) && !empty($userDetails['day_date'])){ ?>
                          <strong>Schedule:</strong> <?=date('d-m-Y', strtotime($userDetails['day_date']));?><br/>
                          <?php } ?>
                        </p>
                        <p style="margin: 0 0 24px;"><a href="<?=$refererURL;?>" style="display: inline; font-size: 14px; line-height: 100%; --text-opacity: 1; color: #7367f0; text-decoration: none;">Click here</a> to view the property</p>
                        <?php if($propertyDetails != false){ ?>
                        <p style="margin: 0 0 24px;"><img src="<?=base_url('uploads/small/'.$propertyDetails[0]->images);?>" width="50%" alt="<?=($propertyDetails != false)?$propertyDetails[0]->address:'';?>" /></p>
                        <?php } ?>
                        <p style="margin: 0 0 24px;">You can reply to this email directly to contact the enquirer.</p>
                        <table style="font-family: 'Montserrat',Arial,sans-serif; width: 100%;" width="100%" cellpadding="0" cellspacing="0" role="presentation">
                          <tr>
